...
Iniciada também a última variante, **XSS DOM**, nível **Low**: confirmado que o payload (`?default=<script>alert('XSS')</script>`) viaja no URL mas nunca é processado pelo servidor — quem o lê (a *source*) e escreve na página sem tratamento (o *sink*) é só o JavaScript do browser. Introduzidos ao glossário os termos **Source** e **Sink**. Explorado também o nível **Medium**, com resultado **parcial**: confirmado um redirecionamento do servidor para `<script>` e a falha do bypass habitual (`<img onerror>`), mas o mecanismo exato ficou por confirmar (dependia do DevTools do Firefox, que não respondeu nesta sessão) — registado honestamente como pergunta em aberto.

Próximo passo: resolver o acesso ao DevTools, confirmar o mecanismo do DOM Medium, e continuar para **High → Impossible**.
