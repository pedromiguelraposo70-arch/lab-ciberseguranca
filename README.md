...
**Dia 4 concluído** (2026-08-12): SQL Injection nível Impossible — o mesmo payload do High foi submetido e **falhou** (resultado vazio), travado pelos *prepared statements*. Fecha-se assim o capítulo do SQL Injection (Low → Medium → High → Impossible), com a lição central de que os "níveis" são versões diferentes do código, não configurações: os prepared statements são uma prática de programação universal, não um "modo". Na mesma sessão, arranque do módulo seguinte do roteiro — **Command Injection**: nível **Low** (injeção via `;` no campo de ping, revelando o utilizador `www-data` e que o alvo corre num container Docker) e nível **Medium** (filtro *blacklist* que apaga `;` e `&&`, contornado com `|` e `&` — ilustrando a fragilidade das blacklists face às whitelists). Primeiro contacto com execução de comandos no SO (RCE), um degrau acima do SQL Injection em impacto.

Próximo passo: Command Injection nível High, e comparação com o Impossible (whitelist na prática).
