...
Ainda na mesma sessão, arranque do módulo **XSS (Cross-Site Scripting)** pela variante **Reflected**, nível Low: injeção de `<script>` no campo "What's your name?", com execução de JavaScript no browser e leitura da cookie de sessão (`PHPSESSID`) — demonstrando o risco de *session hijacking*. Mudança de paradigma face aos módulos anteriores: a vítima passa a ser o **browser de outro utilizador**, não o servidor. Ligação direta ao HttpOnly (a cookie era legível por não ter essa flag, já detetado no nmap da Entrada #8).

Próximo passo: XSS Reflected (Medium → Impossible), e depois as variantes Stored e DOM.
