...
### Consequência para a vítima / organização real — cadeia FTP anónimo → RCE via Apache (Entrada #60)

Ao contrário de um ataque que acontece todo dentro do mesmo serviço, aqui a investigação exige cruzar dois registos que, por defeito, não têm qualquer ligação entre si — o log do FTP (onde o ficheiro foi escrito) e o log do Apache (onde foi executado), muitas vezes geridos por equipas ou ferramentas diferentes. Sem algo que correlacione eventos entre serviços (o papel de um SIEM, por exemplo), uma equipa que investigue comportamento estranho no Apache vê apenas o momento da execução — não tem motivo automático para ir procurar no FTP um upload de dias antes. Isto atrasa a reconstrução da cadeia completa do ataque e, tal como na Entrada #40, dificulta ter a certeza de que todos os ficheiros maliciosos foram encontrados: aqui o problema não é remover um shell já identificado, é sequer chegar a identificá-lo, porque a pista inicial está espalhada por um sistema que ninguém pensou em ligar ao outro.
...
### Consequência para a vítima / organização real — nota sobre o alerta do Optionsbleed (Entrada #62), mesmo sem exploração confirmada

Mesmo sem exploração confirmada, o alerta não pode simplesmente desaparecer. Em qualquer programa de gestão de vulnerabilidades com alguma maturidade (exigido, por exemplo, por normas como ISO 27001 ou PCI-DSS), cada item assinalado por um scanner tem de chegar a um de três destinos documentados: corrigido, mitigado, ou formalmente aceite como risco — nunca simplesmente ignorado. Mesmo esta conclusão de "não é explorável aqui" tem de ficar escrita, com quem a validou e como, porque sem esse registo a mesma investigação teria de ser refeita do zero da próxima vez que o scanner assinalar a mesma CVE — e, mais importante, é esse registo que permite detetar se as condições mudarem no futuro (por exemplo, se alguém introduzir mais tarde um `.htaccess` com diretivas `Limit` contraditórias, o gatilho real do Optionsbleed) e o risco deixar de estar de facto neutralizado.
...
### Consequência para a vítima / organização real — síntese para a partilha Samba anónima (Entrada #64)

Ao contrário dos módulos DVWA anteriores, expostos por natureza a quem estiver na rede onde correm, esta partilha Samba fica normalmente escondida atrás do perímetro da rede interna de uma empresa — não é pensada para ser alcançada de fora. Mas "só acessível de dentro" não é sinónimo de "segura": basta um único ponto de entrada em qualquer lado da rede (um portátil infetado por phishing, um estagiário, um fornecedor com acesso VPN) para que esta partilha sem password fique tão exposta como se estivesse na internet pública. E ao contrário de um scanner de internet, que precisa de varrer milhares de milhões de endereços, alguém já dentro de uma rede interna típica (algumas centenas de máquinas) consegue enumerar todas as partilhas abertas em minutos — é exatamente este pressuposto ("depois do perímetro, está tudo em confiança") que a segmentação interna e o modelo de zero trust tentam eliminar.
...
### Consequência para a vítima / organização real — síntese de fecho da Fase 4 (Entradas #59-60, #64, #65)

Esta entrada fecha um padrão que atravessa toda a Fase 4: nesta mesma VM foram confirmados três acessos independentes — FTP anónimo (Entradas #59-60), partilha Samba anónima (Entrada #64), e agora esta base de dados com credenciais fracas (Entrada #65). Tal como uma casa com três portas destrancadas (frente, traseiras, garagem), corrigir apenas a mais óbvia das três falhas — por exemplo, desativar o acesso anónimo ao FTP — não torna o servidor seguro: um atacante só precisa de encontrar uma porta aberta, enquanto quem defende tem de se lembrar de trancar todas. Isto é particularmente relevante em auditorias de segurança reais, onde é comum encontrar múltiplas falhas independentes no mesmo host — corrigir apenas a que gerou o alerta mais visível, sem procurar ativamente pelas outras, deixa uma falsa sensação de segurança.
...
**Consequência para uma organização real (perspetiva vítima):** Cada uma destas sondagens, isolada, é indistinguível de tráfego legítimo — uma ferramenta de monitorização interna, um scanner de vulnerabilidades da própria empresa, ou simplesmente ruído de fundo típico de qualquer rede ligada à internet. Reagir a cada consulta LDAP RootDSE ou cada tentativa de sessão SMB nula geraria um volume de alertas impossível de gerir, quase todos falsos positivos. O sinal real só aparece na correlação: o mesmo IP de origem a encadear scan de portas, consulta LDAP, sessão SMB nula e tentativa de transferência de zona DNS num curto espaço de tempo — é essa sequência, não qualquer ação isolada, que uma ferramenta como o Wazuh (ou qualquer SIEM) está pensada para detetar. Uma empresa sem essa camada de correlação simplesmente não vê este reconhecimento a acontecer, mesmo tendo todos os logs individuais disponíveis.
...
**Consequência para uma organização real (perspetiva vítima):** O resultado "Path not found" é uma fotografia da estrutura de permissões neste momento exato, não uma garantia permanente. Um domínio Active Directory tende a acumular pequenas alterações ao longo do tempo — uma permissão temporária concedida para resolver um pedido pontual, uma conta adicionada a um grupo para um projeto específico — cada uma perfeitamente razoável isoladamente, e raramente revertida depois de deixar de ser necessária. Ao fim de alguns meses, essa acumulação pode abrir um caminho de escalação que não existia no dia da auditoria, sem que ninguém tenha tomado uma única decisão obviamente errada. Uma empresa que corra esta análise uma vez e trate o resultado como válido para sempre está a confiar numa fotografia cada vez mais desatualizada — o valor real do BloodHound só se mantém com análises repetidas ao longo do tempo, não com uma verificação isolada.
...
**Consequência para uma organização real (perspetiva vítima):** Ao contrário de um ataque de força bruta, onde cada tentativa falhada é mais uma oportunidade de deteção, o Kerberoasting oferece só um momento observável: o pedido do ticket de serviço em si (Evento 4769). Passado esse instante, o hash sai para o computador do atacante e a quebra acontece offline, sem qualquer ligação de rede ao domínio — não há tentativas de login, não há tráfego adicional, não há segunda oportunidade. Isto significa que uma empresa sem deteção especificamente desenhada para esse evento único (por exemplo, sinalizar pedidos de TGS com cifra RC4 numa conta de serviço) fica cega ao ataque do princípio ao fim — não por falta de logs, mas por ninguém ter olhado para o momento certo.
...
**Consequência para uma organização real (perspetiva vítima):** Ao contrário do Kerberoasting da sessão anterior, que já exigia pelo menos uma conta de domínio válida (ainda que de baixo privilégio) para pedir o ticket, aqui não é preciso absolutamente nada — nenhuma credencial, nenhum acesso prévio à rede, só o nome de uma conta candidata. Isto significa que este ataque pode ser tentado por alguém totalmente externo, que nunca conseguiu um phishing bem-sucedido nem comprometeu nenhuma outra máquina — reduz o "custo de entrada" a zero, ao ponto de a fase de acesso inicial deixar de ser sequer necessária antes desta tentativa. Na prática, como o próprio domínio deste lab confirma, a pré-condição (`DoesNotRequirePreAuth` ativo nalguma conta) raramente existe em ambientes bem geridos — mas onde existe, é o único destes ataques da Fase 6 que um atacante pode tentar no dia zero, sem qualquer trabalho preparatório.
...
7. Filtro `agent.name: windows-server` aplicado. Números quase idênticos aos agregados: **17 873 eventos totais** (vs. 18 328 sem filtro), **140 alertas nível 12+** (vs. 146), **1 falha de autenticação**, **7 933 autenticações com sucesso**. Confirma que o `windows-server` é responsável por praticamente todo o volume observado (~97%) — não há diluição relevante de outros agentes a corrigir. O gráfico "Top 10 MITRE ATT&CKs" mantém a mesma forma, ainda dominado por "Valid Accounts".

**Próximos passos (dentro desta sessão):** Já isolado o agente correto; falta agora procurar diretamente pelos IDs de evento Kerberos específicos dos ataques (4768 = pedido de TGT/AS-REQ, 4769 = pedido de TGS — relevante para Kerberoasting, 4771 = falha de pré-autenticação Kerberos) na aba "Events", para perceber se o dado bruto chegou a ser recolhido pelo Sysmon/Wazuh, mesmo que sem alerta dedicado.

8. Pesquisa `data.win.system.eventID: 4769` (Events, últimos 5 dias, agente `windows-server`): **79 resultados**, mas todos com `rule.description: "Windows Logon Success"` (`rule.id 60106`) — descrição genérica que normalmente se associa ao 4624, levantando dúvida se são mesmo eventos 4769. A mesma pesquisa para `4768` devolveu **0 resultados**.
9. Hipótese testada: política de auditoria do Windows desligada para "Kerberos Authentication Service" (subcategoria que controla o 4768). **Descartada** — `auditpol /get /category:"Account Logon"` no `windows-server` confirma as duas subcategorias relevantes a `Success`: `Kerberos Service Ticket Operations` e `Kerberos Authentication Service`. O Windows está a gerar ambos os eventos.
10. Hipótese seguinte testada: canal `Security` não estar configurado no agente Wazuh do `windows-server`. **Também descartada** — o `ossec.conf` do agente (`C:\Program Files (x86)\ossec-agent\ossec.conf`) tem um `<localfile>` para `Security` via `eventchannel`, com uma `<query>` que só exclui eventos de acesso a objetos/ficheiros (5145, 5156, 4656, 4658, 4663, 4660, 4670, 4690, 4703, 4907, 5152, 5157) — nem o 4768 nem o 4769 estão na lista de exclusão, por isso o canal deveria estar a entregá-los ao Wazuh Manager normalmente.
11. Nova hipótese de trabalho: o Windows gera os dois eventos, e o agente lê o canal `Security` sem os excluir — mas o `wazuh-alerts` (o índice que a aba "Events" pesquisa) só guarda eventos que **disparam pelo menos uma regra** do ruleset do Manager; eventos lidos mas sem regra correspondente nunca chegam a "alerta" e não aparecem aqui (ficariam só no `archives`, módulo tipicamente desativado por defeito). Isto explicaria o 4768 a zero (nenhuma regra do ruleset por defeito reconhece um simples pedido de TGT) e o 4769 a aparecer "disfarçado" de "Windows Logon Success" (uma regra genérica de logon bem-sucedido, que não distingue um pedido de TGS legítimo de rotina de um padrão de Kerberoasting).

12. Evento expandido e confirmado: `data.win.system.eventID: 4769` genuíno, mas `targetUserName: WIN-54OBK8B48L5$@LAB.LOCAL` — a própria conta de computador do servidor, tráfego Kerberos de rotina, sem qualquer relação com o Kerberoasting da Sessão 6.3. Classificado sob `rule.id 60106` ("Windows Logon Success"), nível 3, `rule.mitre.technique: Valid Accounts` — a mesma regra genérica que qualquer logon normal do domínio.

**Resultado:** Diagnóstico confirmado. **Por defeito, o Wazuh não deteta nem Kerberoasting nem AS-REP Roasting:**
- **Kerberoasting (4769):** o evento chega e é indexado, mas cai numa regra genérica de "logon com sucesso" (nível 3) que não distingue um pedido de TGS de rotina (ex.: a própria máquina a pedir um ticket para si) de um pedido suspeito (uma conta de utilizador a pedir um ticket para um SPN de serviço). Fica tecnicamente "visível", mas invisível na prática — indistinguível do ruído de fundo sem inspeção evento a evento.
- **AS-REP Roasting (4768):** o Windows gera o evento (auditoria confirmada a `Success`) e o agente recolhe o canal `Security` sem o excluir, mas nenhuma regra do ruleset por defeito o reconhece — nunca chega a alerta indexado, mesmo tendo sido recolhido.

**Deduções e raciocínio:** Isto repete, com uma nuance diferente, a lição da Entrada #86 (FTP/Fase 5): "instalado" não é o mesmo que "a detetar". Ali a lacuna era um caminho de ficheiros fora do FIM; aqui é mais subtil — a fonte de dados está corretamente configurada (auditoria + canal + agente), mas o **ruleset por defeito não tem lógica para diferenciar Kerberos normal de Kerberos suspeito**. É uma lacuna de deteção comportamental, não de recolha.

13. Confirmação direta no ruleset do Wazuh Manager (`/var/ossec/ruleset/rules/`, via SSH à VM Wazuh): `sudo grep -rn "4768\|4769" /var/ossec/ruleset/rules/` mostra o `4769` presente em duas regras (`0220-msauth_rules.xml` e `0580-win-security_rules.xml`), sempre agrupado com IDs de logon genérico e antigos (`528`, `540`, `673`, `675`, `681`, `4624`) — nenhuma menção a "kerberos" em lado nenhum. O **`4768` não aparece em nenhuma regra do ruleset inteiro**, confirmando de forma definitiva (não só por dedução) que não existe deteção alguma, por defeito, para pedidos de TGT.

**Decisão desta sessão:** Vale a pena criar regras Wazuh próprias — pelo menos uma que dê visibilidade ao 4768 (hoje completamente invisível), e idealmente uma regra de correlação para o 4769 que distinga um padrão de Kerberoasting (várias SPNs diferentes pedidas pela mesma conta de utilizador, num curto espaço de tempo) do ruído de rotina (a própria máquina a pedir tickets para si). Decidido **não** escrever/testar a regra nesta sessão — implica reiniciar o Wazuh Manager e repetir os dois ataques para validar a deteção, um bloco de trabalho com identidade própria que fica para uma sessão dedicada (proposta: Sessão 6.6).

**Consigo explicar isto a alguém?** Sim.

**Como nos podemos defender / lição operacional:** Instalar um SIEM e ligar os agentes não chega — é preciso verificar, evento a evento, se o ruleset por defeito reconhece mesmo os comportamentos relevantes para o teu ambiente. Aqui a auditoria e a recolha estavam corretas; o que faltava era o "cérebro" de deteção ter lógica para eventos Kerberos específicos, em vez de só reconhecer o mínimo denominador comum de "logon com sucesso".

**Gravidade e impacto real (num cenário empresarial):** Numa organização real, isto significa que Kerberoasting e AS-REP Roasting — dois dos ataques mais comuns e eficazes contra Active Directory — podem correr sem gerar um único alerta relevante, mesmo com um SIEM instalado e agentes corretamente configurados. Um atacante com acesso de baixo nível pode escalar silenciosamente, e a equipa de segurança só descobre o incidente depois de o dano estar feito (ou nunca chega a descobrir). É por isto que equipas de deteção maduras não confiam no ruleset "de fábrica" para ambientes de Active Directory — regras específicas para Kerberos são consideradas essenciais, não opcionais.

**Consequência para uma organização real (perspetiva vítima):** Todos os indicadores de saúde do SIEM (agentes ativos, milhares de eventos a fluir, dashboards sem alertas críticos) sugerem proteção, mas medem apenas que o sistema está a funcionar, não que está a detetar o que interessa. A única forma real de o confirmar é simular deliberadamente o ataque exato e verificar se dispara alerta — exatamente o exercício feito nesta sessão. A maioria das organizações nunca faz este teste, porque parece desnecessário quando tudo no dashboard já parece saudável; o resultado é uma dívida de deteção silenciosa, que se acumula sem qualquer sinal visível, e que só se revela quando um atacante real explora exatamente a lacuna nunca testada — nessa altura, já não é uma descoberta de auditoria, é um incidente em curso.

**Domínios relacionados:** CEH D3 (System Hacking), Security+ D4 (Operações de Segurança — deteção e resposta), MITRE ATT&CK (T1558.003 — Kerberoasting, T1558.004 — AS-REP Roasting, na perspetiva de deteção em vez de ataque).

**Próximos passos:** Sessão 6.6 — desenhar, escrever e testar regras Wazuh próprias para o 4768 e o 4769 (repetindo os ataques de Kerberoasting e AS-REP Roasting para validar a deteção).

**English summary:** Reviewed what Wazuh actually captured during the Active Directory attack sessions (6.1-6.4) and found a classic "installed ≠ detecting" gap — this time behavioral rather than a missing collection path. Both attacks hinge on Kerberos ticket-request events (4768 for AS-REP Roasting, 4769 for Kerberoasting). Windows auditing and the Wazuh agent's event channel configuration were both correct, but the default Wazuh ruleset has no Kerberos-aware logic: 4769 gets folded into a generic "successful logon" rule alongside routine machine-account background traffic, and 4768 matches zero rules in the entire ruleset — confirmed directly by grepping the manager's rule files, not just inferred. Decision: worth writing custom detection rules, deliberately deferred to a dedicated future session to allow proper testing (rule authoring, manager restart, re-running both attacks to validate detection).

---


## Entrada #94 — Sessão 6.6: Escrever e testar regras Wazuh próprias para Kerberoasting e AS-REP Roasting

**Máquinas ligadas:** VM Wazuh (`192.168.10.30`), Windows Server (Controlador de Domínio, `192.168.10.1`), Kali Linux (`192.168.10.10`)

**Objetivo:** Fechar a lacuna de deteção confirmada na Entrada #93 — o ruleset por defeito do Wazuh não reconhece o evento 4768 (pedido de TGT, relevante para AS-REP Roasting) nem distingue o 4769 (pedido de TGS, relevante para Kerberoasting) de tráfego Kerberos de rotina. Escrever regras próprias em `local_rules.xml` no Wazuh Manager, reiniciar o serviço, e validar repetindo os dois ataques (Sessões 6.3 e 6.4) para confirmar que passam a gerar alertas distintos e identificáveis.

**Ação executada (sessão em curso):**
1. Revistas as regras existentes que já mencionam o evento 4769 (`0580-win-security_rules.xml` e `0220-msauth_rules.xml`, via `grep` por SSH à VM Wazuh). Confirmado o padrão: regras filtram pelo campo `win.system.eventID` (maiúsculas/minúsculas exatas) e encadeiam-se por `if_sid` a uma regra-mãe — a 60106 ("Windows Logon Success") é filha da 60103, a 60131 ("Windows DC Logon Failure") é filha da 60104.
2. Identificadas as regras-mãe: `60103` ("Windows audit success event", filha da `60001`) e `60104` ("Windows audit failure event", também filha da `60001`). Decidido que a regra nova do 4768 deve ser filha da `60103` — um pedido de TGT bem-sucedido gera sempre um evento de sucesso (uma falha de pré-autenticação gera antes um 4771, não um 4768 com falha).
3. Escrita a regra nova (ID `100010`, nível 5) em `/var/ossec/etc/rules/local_rules.xml`, num grupo próprio (`local,windows,active_directory,kerberos,`), dando ao evento 4768 uma descrição dedicada ("Kerberos: Pedido de TGT (Authentication Service Request)") em vez de ficar sem regra nenhuma, mapeada ao MITRE T1558.004. Confirmado por `tail` que o XML ficou bem formado, sem partir o grupo de exemplo já existente no ficheiro. Nota importante: esta regra dá visibilidade a **qualquer** pedido de TGT (tráfego normal e frequente no domínio) — é uma regra de base, não ainda um detetor específico de AS-REP Roasting.
4. Wazuh Manager reiniciado (`systemctl restart wazuh-manager`) — confirmado `active (running)`. Configuração validada diretamente com `sudo /var/ossec/bin/wazuh-analysisd -t` (sem output, `echo $?` = `0`) — forma determinística de confirmar que a regra nova não tem erro de sintaxe, sem depender de timestamps no log (que estavam poluídos por entradas antigas de "ruleset" do SCA a coincidir com o padrão de busca "rule").

5. Repetido o AS-REP Roasting a partir do Kali (mesmo comando `impacket-GetNPUsers` da Sessão 6.4) — hash da `svc_legacy` obtido com sucesso outra vez, confirmando o ataque. No Wazuh Dashboard (Threat Hunting > Events), a pesquisa por `data.win.system.eventID: 4768` no agente `windows-server` continuou a devolver zero resultados, mesmo alargando a janela temporal a 24 horas e depois a 4 semanas — descarta problema de intervalo de tempo.
6. Verificado em paralelo `data.win.system.eventID: 4769` na mesma janela: 24 hits, todos ainda sob a regra generica `60106`. Confirma que o agente e o canal `Security` continuam operacionais - o problema e especifico ao 4768, nao uma desconexao geral do agente (como aconteceu na Sessao 6.5 com outros agentes).
7. Verificacao direta na origem, via PowerShell no Windows Server (`Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4768; ...}`): o evento 4768 foi mesmo gerado, com um sinal de ataque muito claro - `Client Address: ::ffff:192.168.10.10` (o Kali) e `Pre-Authentication Type: 0` (vs. `2` num pedido normal do Administrator a partir do proprio servidor, `::1`). Confirma que a lacuna nao esta na geracao do evento pelo Windows - esta algures entre o agente Wazuh e a regra.
8. Tentativa de isolar a regra com `wazuh-logtest`, alimentado com um evento 4768 sintetico (JSON manual via `echo | wazuh-logtest`). Resultado inconclusivo: o log ficou decodificado pelo decoder generico `json` (Fase 2), nunca chegando a Fase 3 (regras). Investigada a causa: a regra-raiz de toda a arvore Kerberos/Windows, `60000`, exige explicitamente `<decoded_as>windows_eventchannel</decoded_as>` - um decoder dedicado, diferente do generico `json` que processou o nosso teste manual. Conclusao: este teste nao prova nem desmente a nossa regra; so prova que um JSON colado a mao nao reproduz o decoder real usado pelo agente Windows.

9. Ativado temporariamente o modulo de archives no Wazuh Manager (`<logall>yes</logall>` e `<logall_json>yes</logall_json>` em `/var/ossec/etc/ossec.conf`, com restart), e repetido o ataque mais uma vez a partir do Kali. Resultado **decisivo**, confirmado diretamente em `/var/ossec/logs/archives/archives.json`: o evento 4768 foi corretamente decodificado pelo decoder `windows_eventchannel` e bateu certo com a regra `100010` — `"rule":{"level":5,"description":"Kerberos: Pedido de TGT (Authentication Service Request) - Evento 4768","id":"100010","mitre":{"id":["T1558.004"],"tactic":["Credential Access"],"technique":["AS-REP Roasting"]}}`. O Wazuh enriqueceu automaticamente com a tatica MITRE "Credential Access" e a tecnica "AS-REP Roasting" a partir do ID indicado na regra. **A regra funciona corretamente.**
10. Apesar da confirmacao acima, o evento continuou a nao aparecer no Dashboard (Threat Hunting > Events), mesmo repetindo o ataque de novo. Investigada a causa: `sudo grep -i "indexer-connector" /var/ossec/logs/ossec.log` mostrou avisos continuos e recentes (`Failed to sync agent '003': No available server`, `IndexerConnector initialization failed... retrying`) — o Wazuh Manager perdeu a ligacao ao servico de indexacao (OpenSearch), pelo que nenhum alerta novo, de nenhuma regra, esta a chegar ao Dashboard neste momento. Confirmado que o servico `wazuh-indexer` em si esta ativo (`systemctl status` = `active (running)`, ha 2 dias) — nao e um servico desligado, e sim uma falha de ligacao entre o Manager e o Indexer, causa ainda por identificar.

**Resultado:** A regra Wazuh para o evento 4768 (visibilidade para pedidos de TGT, cobrindo o padrao de AS-REP Roasting) foi escrita, testada e **confirmada a funcionar corretamente** — validada diretamente no `archives.json` do Manager, com decoder correto, regra correspondida e enriquecimento MITRE automatico. A validacao pelo Dashboard nao foi possivel nesta sessao devido a um problema de infraestrutura novo e nao relacionado: o Manager perdeu a ligacao ao Indexer, pelo que nenhum alerta novo esta a ser mostrado no Dashboard, independentemente da regra.

**Deducoes e raciocinio:** Distinguir "a regra nao funciona" de "a plataforma nao esta a mostrar o resultado" so foi possivel por testar em duas camadas independentes: o `archives.json` (o que o `analysisd` decide na origem, sem depender do resto do pipeline) versus o Dashboard (o produto final, que depende tambem do indexador e da ligacao entre os dois). Sem essa separacao, seria facil concluir erradamente que a regra estava mal escrita, quando na verdade estava correta desde o inicio. Fica tambem uma licao de arquitetura: mesmo com o Manager e o Indexer ambos "ativos" individualmente, a ligacao entre os dois e um terceiro ponto de falha que precisa de verificacao propria.

**Decisao desta sessao:** Fechar a Sessao 6.6 com o objetivo do 4768 cumprido e validado. A regra de correlacao para o 4769 (distinguir Kerberoasting de trafego de rotina) fica por escrever — so faz sentido depois de o Dashboard voltar a mostrar alertas em tempo real, para permitir validacao visual. A investigacao a ligacao Manager -> Indexer fica registada como pendencia nova, para uma sessao dedicada.

**Consigo explicar isto a alguem?** Sim.

**Como nos podemos defender / licao operacional:** Num SIEM em producao, a saude do pipeline completo (agente -> manager -> indexador -> dashboard) precisa de ser monitorizada em cada elo, nao so verificar se os servicos individuais estao "up". Uma equipa que so confia no dashboard, sem monitorizar o proprio SIEM, pode perder semanas de alertas sem perceber — exatamente o tipo de falha silenciosa encontrada aqui.

**Gravidade e impacto real (num cenario empresarial):** Se isto acontecesse numa organizacao real sem ser notado, seria um "blind spot" total e silencioso: as regras de deteccao podem estar corretissimas, os agentes a reportar sem falhas, e mesmo assim a equipa de seguranca nao veria absolutamente nada no dashboard, sem qualquer aviso obvio de que o problema nao e "nao ha ataques" mas sim "o SIEM parou de mostrar". E precisamente o cenario que permite a um atacante operar durante dias ou semanas sem deteccao nenhuma, mesmo num ambiente aparentemente bem instrumentado.

**Consequência para uma organização real (perspetiva vítima):** A decisão mais provável perante um Dashboard sem resultados é exatamente essa — não fazer nada, porque uma tela vazia e um "não há ameaça" são visualmente indistinguíveis. Nesta sessão, só foi possível desconfiar do "zero resultados" porque havia uma certeza externa de que o ataque tinha mesmo sido executado minutos antes — essa certeza é que justificou insistir e acabar por confirmar os dados diretamente na API do Indexer. Um analista de SOC real, a rever um alerta de origem desconhecida (não um teste seu), não tem essa certeza prévia: uma pesquisa vazia não lhe dá motivo nenhum para duvidar da própria ferramenta. O resultado é um alerta que existe, gerado corretamente e já indexado, mas que se perde por completo — não porque a deteção falhou, mas porque uma definição de interface (uma janela de tempo por defeito) decidiu, sem ninguém notar, que ninguém precisava de o ver.

**Dominios relacionados:** Security+ D4 (Operacoes de Seguranca — monitorizacao e manutencao de ferramentas de deteccao), CEH D3 (perspetiva de deteccao), MITRE ATT&CK (T1558.004 — AS-REP Roasting, na perspetiva de deteccao).

**Proximos passos:** Diagnosticar a ligacao Manager -> Indexer (sessao dedicada); so depois escrever e validar a regra de correlacao do 4769 para Kerberoasting.

**English summary:** Wrote and validated a custom Wazuh rule giving visibility to Windows Event 4768 (Kerberos TGT requests), the event central to detecting AS-REP Roasting. The Dashboard repeatedly showed zero results even after the rule was added and the attack repeated, which looked like a broken rule — but direct inspection of the manager's archives.json proved otherwise: the rule matched correctly, with the right decoder and automatic MITRE ATT&CK enrichment. The real cause was a separate, unrelated infrastructure problem: the Wazuh Manager had lost its connection to the indexing service, so no new alerts from any rule were reaching the Dashboard, even though both the Manager and the Indexer services were individually running. Session closed with the 4768 rule objective met and validated at the source; the Kerberoasting (4769) correlation rule and the Manager-to-Indexer connectivity issue are both deferred to a future session.

**Nota de continuação (mesma sessão, mais tarde):** A investigação continuou depois do fecho acima, e revelou que a pista do indexer-connector, apesar de real, não era a causa do problema do Dashboard:
11. Corrigido `<host>https://0.0.0.0:9200</host>` para `<host>https://192.168.10.30:9200</host>` em `/var/ossec/etc/ossec.conf` (endereço invalido como destino de ligacao; confirmado o IP correto via `openssl x509 ... -ext subjectAltName` no certificado do Filebeat, que so e valido para `192.168.10.30`). Apos restart, os avisos `indexer-connector` deixaram de aparecer (`INFO: IndexerConnector initialized successfully` para todos os indices).
12. Repetido o ataque varias vezes apos a correcao; o Dashboard continuou em zero resultados. Confirmado por `archives.json` que a regra `100010` continua a disparar corretamente para cada evento 4768 novo (incluindo um verificado diretamente na origem via `Get-WinEvent` no Windows Server, para descartar duvidas de sincronizacao temporal entre VMs).
13. **Descoberta importante:** os indices que o `indexer-connector` corrigiu (`wazuh-states-inventory-*`, `wazuh-states-vulnerabilities-*`) sao de **dados de inventario/estado** (FIM, vulnerabilidades, syscollector) — um mecanismo separado dos **alertas de seguranca**. Os alertas que aparecem em Threat Hunting -> Events sao entregues por um servico distinto, o **Filebeat**, que le o `alerts.json` do Manager e envia para o indice `wazuh-alerts`. Ou seja, a correcao ao `indexer-connector` era real e válida, mas nao era a causa do problema original.
14. Verificado o Filebeat: servico `active (running)` ha 2 dias; `sudo filebeat test output` confirma ligacao bem-sucedida ao indexador (TLS OK, handshake OK, versao 7.10.2). Mas `grep -A5 -i "paths" /etc/filebeat/filebeat.yml` nao devolveu nada — o ficheiro principal nao tem nenhum caminho de input configurado. Hipotese de trabalho: o Wazuh normalmente usa um **modulo** proprio do Filebeat (`/etc/filebeat/modules.d/wazuh.yml`) para apontar ao `alerts.json`, em vez de um input direto no `filebeat.yml`. Por confirmar: se esse modulo esta ativo e bem configurado.
...
**Nota de continuação (dia seguinte, 2026-09-07): Resolução final.**

15. Retomada a investigação, confirmando primeiro o módulo Filebeat: `sudo filebeat modules list` devolveu "Enabled modules/filesets: (), wazuh (alerts)" — módulo ativo. O manifest do módulo (`/usr/share/filebeat/module/wazuh/alerts/manifest.yml`) confirma `var.paths` por defeito `/var/ossec/logs/alerts/alerts.json`, coincidindo com a realidade do sistema. `sudo filebeat keystore list` mostrou as entradas `username`/`password` presentes, e nenhuma pesquisa por erros (401/403/error) no log do Filebeat devolveu resultados.
16. O alerta `rule.id: 100010` foi encontrado diretamente dentro de `alerts.json` via `grep` — confirma, mais uma vez, que o Manager grava o alerta corretamente no ficheiro que o Filebeat lê.
17. Nesta altura o Pedro sinalizou que a abordagem estava a ficar circular (`grep` após `grep`, sem avançar para uma conclusão) — decisão conjunta de mudar de método em vez de continuar a inspecionar configuração estática.
18. `systemctl restart filebeat`, com captura do log de arranque completo: confirmado "Harvester started for file: /var/ossec/logs/alerts/alerts.json" e teste de output com ligação TLS bem-sucedida. O registry do Filebeat (`/var/lib/filebeat/registry/filebeat/log.json`) mostrou o offset de leitura a avançar de forma contínua e substancial ao longo do tempo, sempre para o mesmo inode do `alerts.json` — ou seja, o Filebeat estava mesmo a ler e a avançar pelo ficheiro, ao contrário da suspeita inicial. Também aqui, nenhum erro.
19. Mudança de estratégia decisiva: em vez de continuar a inspecionar o Filebeat por fora, testado diretamente o Wazuh Indexer via API REST — `curl -k -u admin "https://192.168.10.30:9200/wazuh-alerts-4.x-*/_count?q=rule.id:100010"`. Primeira tentativa deu `Unauthorized` (a password do Dashboard não coincidia com a do utilizador `admin` do indexer, e não havia registo guardado da password original de instalação). Usada a ferramenta oficial `wazuh-passwords-tool.sh -u admin -p '<password-nova>'` para definir uma password conhecida — a própria ferramenta atualizou automaticamente o keystore do Filebeat com as novas credenciais, e avisou explicitamente que a password do Dashboard também mudava (é o mesmo utilizador/sistema de segurança). Repetido o `curl` com a password nova: **`"count":11`** — prova definitiva de que os 11 alertas do `rule.id 100010` já estavam gravados no índice `wazuh-alerts-4.x-*` desde o dia anterior, com timestamps espalhados por quase 24 horas.
20. Esta descoberta eliminou de vez a hipótese de "os dados não chegam": confirmou que a regra, o Filebeat e o indexer-connector (já corrigido) sempre funcionaram corretamente. O problema só podia estar na camada de visualização (Dashboard), não na pipeline de dados.
21. Login feito no Dashboard com a password nova. Repetida a pesquisa por `rule.id: 100010`, desta vez com o intervalo de tempo alargado para **"Last 24 hours"** (em vez do intervalo curto, provavelmente "Last 15 minutes" por defeito, usado em todas as tentativas anteriores desde a Entrada #93): **9 eventos apareceram de imediato**, com a descrição correta da regra ("Kerberos: Pedido de TGT (Authentication Service Request) - Evento 4768"), nível 5. Confirmado visualmente pelo Pedro via screenshot. (Nota: 9 em vez dos 11 do `curl` porque a vista tinha um filtro `agent.name: windows-server` e a janela de 24h não cobria os dois eventos mais antigos, de véspera à noite — não é uma discrepância nova, é o esperado.)

**Resultado final (2026-09-07): RESOLVIDO.** O evento 4768 aparece no Dashboard, cumprindo exatamente o critério exigido pelo Pedro. Causa raiz real: o intervalo de tempo selecionado no Dashboard estava sempre demasiado curto para cobrir o momento dos ataques de teste — nunca foi um problema da regra, do Filebeat, ou do indexer-connector. As correções feitas a essas duas últimas peças foram reais mas laterais, tal como já estava registado acima — não eram a causa, mas também não foram tempo desperdiçado: confirmaram de forma rigorosa que a pipeline de dados estava saudável, o que por eliminação apontou para a única camada que restava por testar (a interface).

**Dedução adicional (lição de metodologia):** a técnica que desbloqueou isto foi contornar por completo a interface do Dashboard e perguntar diretamente à API do Wazuh Indexer se o documento existia (`curl` autenticado, `_count`/`_search` no índice). Isto separa de forma definitiva e rápida "os dados não chegam" de "os dados chegam mas a UI não os mostra" — devia ser um passo de diagnóstico precoce, não um último recurso, sempre que um Dashboard parecer não mostrar algo que devia.

**Nota de segurança prática:** a password do utilizador `admin` do Wazuh Indexer (e, por consequência, do login do próprio Wazuh Dashboard) foi alterada nesta sessão, como efeito colateral necessário para fazer o teste direto à API — não havia registo da password original de instalação. O Filebeat foi resincronizado automaticamente pela própria ferramenta; o login do browser teve de ser atualizado manualmente com a password nova.

**Decisão final da Sessão 6.6:** sessão fechada e validada, com o critério de sucesso definido pelo Pedro cumprido — o evento 4768 aparece no Dashboard. A regra de correlação para o 4769 (Kerberoasting) fica como próximo passo natural para uma sessão futura.

**English summary (continuação, 2026-09-07):** Resumed the investigation the next day. Confirmed the Filebeat wazuh module was correctly enabled and configured, with no authentication or connection errors anywhere in its logs, and its registry showing the harvester actively advancing through alerts.json — ruling out every plausible pipeline failure one by one. The breakthrough came from bypassing the Dashboard UI entirely and querying the Wazuh Indexer's REST API directly for the alert count, which required first setting a known password for the indexer's admin account (a side effect: this also changed the Dashboard's login password, requiring a manual update on the browser side). The query returned a non-zero count, proving the alerts had been sitting in the index the whole time. The dashboard's default time-range filter — not any backend component — had been hiding them from every previous search. Widening it to "Last 24 hours" made all nine matching events appear immediately. Root cause: a UI-level filter, not a data pipeline defect. Session 6.6 formally closed and validated against the user's own success criterion (the event visibly appearing in the Dashboard).

## Entrada #95 — Sessão 6.7: Regra de correlação Wazuh para Kerberoasting (Evento 4769)

**Máquinas ligadas:** VM Wazuh (`192.168.10.30`), Windows Server (Controlador de Domínio, `192.168.10.1`), Kali Linux (`192.168.10.10`)

**Objetivo:** Fechar a última lacuna de deteção da Fase 6 — o ruleset por defeito do Wazuh (regra genérica `60106`) classifica todo o evento 4769 (pedido de TGS) como "Windows Logon Success", sem distinguir um padrão de Kerberoasting de tráfego de rotina. Escrever uma regra própria que identifique especificamente o sinal de ataque.

**Ação executada:**
1. Recolhida uma amostra real de um evento 4769 de tráfego rotineiro (conta de máquina `WIN-54OBK8B48L5$` a pedir um ticket para si própria, a partir de `::1`, com `ticketEncryptionType: 0x12` — AES256) via `archives.json`, para servir de referência de "normal".
2. Repetido o ataque de Kerberoasting a partir do Kali (`impacket-GetUserSPNs` contra a `svc_sql`, o mesmo da Sessão 6.3/6.4) e recolhida a amostra real do evento 4769 gerado pelo ataque: conta `uteste@LAB.LOCAL` a pedir a SPN `MSSQLSvc/sql01.lab.local:1433` (`svc_sql`), a partir de `::ffff:192.168.10.10` (Kali), com **`ticketEncryptionType: 0x17` (RC4)**.
3. Identificado o sinal de deteção: o `ticketEncryptionType` é o diferenciador mais forte e imediato — RC4 (`0x17`) é criptograficamente fraco e é exatamente o que o Kerberoasting explora para crackar hashes offline (confirmado já na Sessão 6.3, onde o hash RC4 crackou em segundos); um domínio bem configurado deveria usar AES (`0x12`) para pedidos de serviço legítimos. Decidido usar este sinal como base da regra, em vez de (ou antes de) uma regra de correlação por frequência — funciona logo no primeiro pedido, não precisa de esperar por um padrão de múltiplos pedidos.
4. Desenhada a regra `100011` (nível 10, filha da `60106`): dispara quando o evento é 4769 **E** `ticketEncryptionType` é `0x17` **E** o `serviceName` não termina em `$` (para excluir contas de máquina e tráfego rotineiro). Mapeada ao MITRE T1558.003 (Kerberoasting).
5. **Incidente durante a escrita da regra:** ao tentar inserir a regra no `local_rules.xml`, o conteúdo de um comando de terminal foi colado por engano diretamente para dentro do ficheiro (em vez de ser executado no terminal), substituindo por completo o conteúdo anterior — o ficheiro ficou com 19 linhas de texto de um comando shell, em vez de XML válido. Detetado a tempo, antes de qualquer reinício do serviço: o Wazuh Manager, a correr em memória, manteve a regra `100010` ativa durante todo o incidente, sem qualquer interrupção real da deteção. Não havia backup do ficheiro original. Reconstruído o ficheiro com as duas regras conhecidas e corretas (`100010` e `100011`); perdeu-se, muito provavelmente, apenas o exemplo genérico que o Wazuh traz por defeito neste ficheiro (uma regra de exemplo para SSH, nunca usada neste laboratório) — sem impacto funcional conhecido.
6. Ficheiro corrigido validado com `sudo /var/ossec/bin/wazuh-analysisd -t` (exit code `0`, sem erros de sintaxe) **antes** de qualquer reinício, para garantir que o serviço não ficava indisponível.
7. Wazuh Manager reiniciado (`systemctl restart wazuh-manager`) — confirmado `active (running)`, com as duas regras (`100010` e `100011`) carregadas.

**Resultado (parcial — sessão pausada aqui por cansaço do Pedro):** A regra `100011` está escrita, com sintaxe validada e o serviço reiniciado com sucesso. **Ainda não foi validada a funcionar** — falta repetir o ataque de Kerberoasting e confirmar em `archives.json` que a regra dispara corretamente, e depois no Dashboard. Isto fica registado como o próximo passo exato para a continuação, não como um objetivo já cumprido.

**Lição de metodologia (reforça a da sessão anterior):** um incidente separado do problema técnico em si — colar o conteúdo errado no sítio errado — voltou a acontecer, desta vez com um custo maior (o ficheiro de regras chegou a ficar completamente inválido, embora sem impacto real porque foi apanhado antes de qualquer reinício). Lição prática: sempre que um comando de terminal for longo ou multi-linha, confirmar explicitamente que se está no prompt do bash e não dentro de um editor, antes de o correr — e nunca reiniciar um serviço depois de editar um ficheiro de configuração sem validar a sintaxe primeiro (`wazuh-analysisd -t`), exatamente o hábito que evitou que este incidente se tornasse numa quebra real de deteção.

**Próximos passos (retomar exatamente aqui):**
1. No Kali: repetir `impacket-GetUserSPNs -request -dc-ip 192.168.10.1 'lab.local/uteste:NovaPassword456!' -outputfile kerberoast_hash.txt`.
2. Na VM Wazuh: `sudo grep '"id":"100011"' /var/ossec/logs/archives/archives.json | tail -3` para confirmar que a regra dispara.
3. Confirmar visualmente no Dashboard (Threat Hunting → Events, `rule.id: 100011`), com o intervalo de tempo alargado (já sabemos, da Sessão 6.6, que o filtro por defeito é demasiado curto).
4. Só depois, fechar formalmente a Sessão 6.7.

**English summary:** Began writing a Wazuh correlation rule for Kerberoasting (Windows Event 4769). Collected real samples of both routine Kerberos traffic (a machine account requesting its own ticket, AES-encrypted) and an actual Kerberoasting attack (impacket's GetUserSPNs requesting a service ticket for a vulnerable account), finding that the ticket's encryption type is a strong, immediate detection signal: RC4 (used by the attack) versus AES (used by routine traffic) — RC4 being exactly what makes the resulting hash crackable offline, as already demonstrated in Session 6.3. Designed rule 100011 around this signal. A copy-paste mistake while editing the rules file briefly left it containing literal shell-script text instead of valid XML; caught before any service restart, so the live, already-loaded ruleset was never actually affected — reconstructed the file from known-good content and validated syntax before restarting. The new rule is written and the service is running, but not yet validated against a live attack — deferred to the next session.

### Nota de continuação (mesmo dia, 2026-09-08): investigação e resolução final

**Resultado final: RESOLVIDO.** A regra `100011` foi confirmada a disparar corretamente, tanto no `archives.json`/`alerts.json` como visualmente no Dashboard (Threat Hunting → Events, `rule.id: 100011`, "Last 24 hours" — 1 resultado, nível 10, descrição correta, timestamp `Sep 8, 2026 @ 17:43:37`).

**O problema real (muito mais interessante do que parecia à primeira vista):** ao retomar a sessão e repetir o ataque, a regra `100011` continuava a não disparar — e, mais surpreendente ainda, nem sequer a regra genérica de fábrica `60106` (que só verifica o `eventID`, nada mais) disparava para este evento específico, apesar de disparar sem problema para outros eventos 4769 do mesmo dia. Ou seja, o evento do ataque estava a desaparecer por completo do `archives.json`, sem nenhuma regra associada — como se nunca tivesse sido avaliado.

**Hipóteses testadas e descartadas, por ordem (fica registado porque o percurso tem tanto valor como a resposta final):**
1. **Base de dados local do MITRE ATT&CK (`mitre.db`) sem o técnica `T1558.003`.** Testado via Python/`sqlite3` diretamente na tabela `technique`. Descartada: a regra `60106` nem sequer tem bloco `<mitre>`, por isso um problema nessa base de dados nunca poderia impedi-la de disparar.
2. **Diferença estrutural no próprio evento.** Comparado o bloco `win.system` completo (channel, severityValue, eventID, etc.) do evento do ataque com o de eventos 4769 rotineiros do mesmo dia — eram estruturalmente idênticos em tudo o que a regra `60106` verifica. Descartada.
3. **A condição `negate` na regra 100011** (`win.eventdata.serviceName` a não terminar em `$`). Removida experimentalmente essa condição, mantendo só `eventID` + `ticketEncryptionType`; o evento continuou a não disparar nem a regra simplificada nem a `60106`. Descartada.
4. **Conflito com um ruleset antigo** (`0220-msauth_rules.xml`, regras `18104`/`18105`/`18107`/`18139`, que também listam o evento 4769). Investigado a fundo, mas estas regras verificam um campo `<status>` legado que o decodificador moderno (`windows_eventchannel`, baseado em JSON) muito provavelmente nunca preenche — ou seja, é improvável que estas regras cheguem sequer a ser avaliadas para os nossos eventos, em qualquer dos casos. Descartada.
5. **Wazuh Manager ainda "a aquecer" depois do reinício.** Testado esperando mais de 11 minutos depois de reiniciar o serviço antes de repetir o ataque — o evento continuou a desaparecer da mesma forma. Descartada.
6. **Crash e reinício automático do `wazuh-analysisd` especificamente ao processar este evento.** Verificado via `systemctl status` — o serviço manteve-se `active (running)` de forma contínua, sem qualquer reinício inesperado. Descartada.

**A causa real:** uma regra de fábrica do próprio Wazuh, a `92651` (ficheiro `0840-win_event_channel.xml`), filha da mesma `60106`, com **nível 0** (silenciosa — nunca gera alerta próprio, mas "reclama" o evento como tratado). Esta regra foi concebida para assinalar logons remotos bem-sucedidos (tipicamente o evento 4624), verificando se o campo `win.eventdata.ipAddress` "parece" um endereço IPv4 normal e não é `127.0.0.1`. O problema: o evento 4769 do nosso ataque tem um IP real do Kali no formato IPv6-mapeado (`::ffff:192.168.10.10`), e a expressão regular da regra `92651` (não ancorada) encontra o `192.168.10.10` embutido dentro dessa notação — logo, dá como "match". Por ser nível 0, o motor de regras do Wazuh trata isto como "evento já resolvido, não procures mais nada" — incluindo a nossa própria regra `100011`, que também é filha da `60106` mas nunca chega a ser avaliada, porque o evento já tinha sido "reclamado" primeiro pela `92651`. Os eventos de rotina (pedidos locais, IP `::1`) não ativam este regex de IPv4, por isso passam despercebidos por esta regra e chegam normalmente à `60106`.

**A correção:** a própria Wazuh já usa este padrão — regras como a `92652` ("possível pass-the-hash via NTLM") são filhas diretas da `92651`, e disparam alerta próprio na mesma, mesmo com um "pai" de nível 0. Seguindo o mesmo padrão, a regra `100011` foi alterada para ser filha da `92651` em vez de diretamente da `60106`, mantendo as condições de `eventID` (`4769`) e `ticketEncryptionType` (`0x17`). Assim, a regra silenciosa de fábrica continua a fazer o seu trabalho normal, mas a nossa regra tem finalmente hipótese de ser avaliada a seguir.

**Validação final:** ataque repetido pela última vez; confirmado no `archives.json` (regra `100011`, nível 10, MITRE `T1558.003`, todos os campos corretos) e, de forma decisiva, confirmado visualmente no Dashboard do Wazuh.

**Lição de metodologia (a mais importante desta sessão):** quando uma regra parece não disparar sem motivo aparente, e as condições da própria regra estão corretas, vale a pena procurar por **outras regras que partilhem o mesmo "pai" (`if_sid`)** — em particular regras de nível 0 (silenciosas), que podem estar a "engolir" o evento antes de chegar à nossa. Uma pesquisa simples (`grep -rl` por termos relevantes em `/var/ossec/ruleset/rules/`) foi o que finalmente revelou o conflito, depois de várias hipóteses mais complexas (base de dados, timing, crashes) terem sido descartadas sem resultado.

**Nova checklist fixada (skill `validar-regra-wazuh`):** por já ter acontecido mais do que uma vez quase declarar uma regra "resolvida" só com base no `archives.json`, ficou formalizado um procedimento fixo: validar sintaxe antes de reiniciar → reiniciar e confirmar `active (running)` → repetir o ataque → verificar em camadas (archives.json → alerts.json → Dashboard, com janela de tempo alargada) → só então declarar "resolvido" → documentar sempre o percurso completo, incluindo as hipóteses descartadas.

**Acrescento (mesmo dia): reintrodução do filtro de exclusão de contas de máquina.** A condição `<field name="win.eventdata.serviceName" negate="yes">\$$</field>` (excluir `serviceName` terminado em `$`, ou seja, contas de máquina) tinha sido removida durante o diagnóstico, para isolar variáveis. Depois de confirmada a causa raiz e a correção, reintroduzida na regra `100011` (agora filha da `92651`); sintaxe validada, manager reiniciado, ataque repetido, e regra confirmada a disparar corretamente com o filtro ativo (`serviceName: svc_sql`, não termina em `$`, passa o filtro sem problema).
...
## Entrada #98 — Sessão 6.9: Balanço e hardening da Fase 6 — desligar LLMNR/NBT-NS/mDNS

**Data:** 2026-09-17

**Máquinas ligadas:** OPNsense (192.168.10.254, gateway/DHCP — ligado primeiro), Windows Server/DC (192.168.10.1, WIN-54OBK8B48L5, `lab.local`), Windows 11 (cliente, IP por DHCP — a vítima), Kali Linux (atacante, 192.168.10.10, `eth0`). Wazuh não envolvido (esta sessão é prevenção, não deteção).

**Objetivo:** Fechar a Fase 6 com o mesmo espírito da Fase 5 — não só atacar, também corrigir. Duas partes: (A) um balanço defensivo, mapeando cada ataque feito na fase à defesa concreta que o teria prevenido ou detetado mais cedo; e (B) implementar e provar pelo menos uma dessas defesas nas VMs. A defesa escolhida foi desligar o LLMNR/NBT-NS, que fecha diretamente o ataque do Responder da Entrada #97.

### Parte A — Balanço defensivo da Fase 6

Para cada ataque feito (6.1 a 6.6), a defesa que o trava ou deteta mais cedo:

- **6.1 — Enumeração sem credenciais:** prevenção limitada (o essencial de um domínio é público por design — nome do domínio, DC, versão — e não se "desliga"; só se fecha o opcional: bind LDAP anónimo, RestrictAnonymous, transferência de zona DNS, SMB signing, tudo já bem configurado neste lab). A defesa principal é **deteção**: correlacionar a *sequência* de reconhecimento (SIEM/Wazuh), já que cada sonda isolada é indistinguível de tráfego legítimo.
- **6.2 — BloodHound:** a ferramenta é neutra — só desenha caminhos que a configuração já permite. Defesa = **menor privilégio e ausência de cadeias tóxicas de permissões**, auditadas ao longo do tempo (incluindo correr o BloodHound defensivamente, como auditor). O "caminho não encontrado" é uma fotografia de um momento, não uma garantia permanente.
- **6.3 — Kerberoasting:** a **prevenção pesa mais** que a deteção, porque a quebra do hash acontece offline, fora da rede (só se vê o pedido, Evento 4769, nunca o sucesso). Defesas: password de serviço forte/aleatória, idealmente **gMSA** (auto-gerada e rotativa), forçar **AES** em vez de RC4, e remover SPNs desnecessários. A regra Wazuh `100011` (4769 com RC4) é aviso precoce/segunda linha.
- **6.4 — AS-REP Roasting:** defesa = **eliminar a pré-condição** — auditar e remover o atributo "não exigir pré-autenticação" (não vem ligado por defeito; zero contas com a flag = ataque impossível). Só se uma conta legada precisar mesmo dele, volta-se ao plano do 6.3 (password forte/gMSA + AES). Deteção (4768, regra `100010`) é secundária, por ser o ataque de "dia zero".
- **6.6 — LLMNR/NBT-NS poisoning (Responder):** defesa = **desligar os mecanismos de fallback de resolução de nomes** (LLMNR, NBT-NS e mDNS) — não é corrigir um bug, é reduzir uma superfície de ataque desnecessária, já que o DNS faz o trabalho normal. Reforço em profundidade: SMB signing (contra NTLM relay) e passwords fortes (se um hash for capturado, a quebra offline falha). Deteção fraca. **Esta foi a defesa implementada na Parte B.**

**Nota sobre 6.7 e 6.8:** o Pass-the-Hash (6.7) e o DCSync/Golden Ticket (6.8) ficaram fora da prática hands-on por decisão própria (ver roteiro e proposta da Fase 6) — cobertos só ao nível conceptual. Ficam, mesmo assim, no balanço defensivo: o Pass-the-Hash mitiga-se com Credential Guard, desativar NTLM a favor de Kerberos e não reutilizar contas de administrador local (LAPS); o DCSync/Golden Ticket, restringindo a permissão de replicação (`Replicating Directory Changes`) só aos DCs, monitorizando o Evento 4662 e rodando periodicamente a password da conta `krbtgt`.

### Parte B — Hardening implementado: desligar LLMNR, NBT-NS e mDNS

Estado inicial confirmado no Windows 11 (o "antes"): LLMNR ligado (sem política `EnableMulticast`), NBT-NS ligado (`TcpipNetbiosOptions = 0`, herdado do DHCP).

1. **LLMNR — por GPO.** Criada a GPO `Hardening-Desligar-LLMNR-NBTNS`, ligada à OU `Lab \ Computadores` (a mesma OU do objeto do Windows 11 e da GPO do aviso de login da Entrada #70 — o LLMNR é *Computer Configuration*, por isso apanha o computador, não o utilizador). Definição: **Computer Configuration -> Policies -> Administrative Templates -> Network -> DNS Client -> "Turn off multicast name resolution" = Enabled**.
2. **NBT-NS — no cliente (limitação honesta).** A definição nativa de GPO "Configure NetBIOS settings" **não existe** nesta versão do Windows Server (ADMX mais antigo — confirmado na lista de 23 definições do DNS Client). Durante anos o NBT-NS só se desligava por script ou opção de DHCP, não por GPO. Como é um único cliente no lab, desligou-se diretamente no adaptador do Windows 11: **ncpa.cpl -> adaptador Ethernet -> Propriedades -> IPv4 -> Propriedades -> Advanced -> separador WINS -> "Disable NetBIOS over TCP/IP"**.
3. **Aplicação e verificação (o "depois").** `gpupdate /force` no Windows 11 + reinício. Confirmado por PowerShell: `EnableMulticast : 0` (LLMNR desligado) e `TcpipNetbiosOptions : 2` (NBT-NS desligado).
4. **1a prova (Responder a correr no Kali, `sudo responder -I eth0`).** No Windows 11, `Win+R -> \\testelab4`. O log do Responder mostrou **apenas** linhas `[MDNS] Poisoned answer sent ... for name testelab4.local` — **zero** linhas `[LLMNR]` ou `[NBT-NS]`. Isto **prova que o LLMNR e o NBT-NS ficaram silenciados**: o Windows 11 deixou de gritar por esses dois canais. Porém, recuou para um terceiro canal — o **mDNS** — que ainda não tínhamos desligado; o Responder envenenou-o e o Windows 11 ainda tentou autenticar-se (ecrã "Introduzir credenciais de rede", cancelado sem introduzir a password real). O Responder indicou "Skipping previously captured hash" porque já tinha o hash da conta desde a #97 (deduplicação), não porque a defesa tivesse impedido a captura.
5. **Descoberta (lição central da sessão): o mDNS é um terceiro vetor residual.** A dupla LLMNR/NBT-NS é o hardening clássico, mas o Windows moderno tem o mDNS como terceiro mecanismo de fallback. Desligado no Windows 11 com `EnableMDNS = 0` (DWORD) na mesma chave de política `HKLM\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient` onde já vive o `EnableMulticast = 0` do LLMNR, seguido de reinício. Confirmado `EnableMDNS : 0`.
6. **2a prova (após desligar o mDNS).** No Windows 11, `Win+R -> \\testelab5`: desta vez o Windows respondeu com **"O Windows não consegue aceder a \\testelab5 — verifique a ortografia"** — o erro limpo de um nome que *não resolve por nenhum canal*, e já **não** o ecrã de "Introduzir credenciais de rede". Contraste direto com o comportamento anterior. **Confirmação final (2026-09-18):** identificadas e terminadas duas instâncias antigas do Responder, presas desde o dia anterior (`ps aux | grep -i responder` revelou dois pares de processos com timestamp `set17`; `sudo kill -9` pelos PIDs, depois de uma primeira tentativa com `pkill -f responder.py` falhar por diferença de maiúsculas/minúsculas no nome do processo). Com a porta livre, `sudo responder -I eth0` arrancou sem erro, à escuta. No Windows 11, `Win+R -> \\testelab6` (nome nunca antes usado nos testes) devolveu diretamente **"O Windows não consegue aceder a \\testelab6 — verifique a ortografia"**, sem qualquer ecrã de credenciais. No terminal do Kali, o log ficou parado em "Listening for events..." — **zero linhas `[LLMNR]`, `[NBT-NS]` ou `[MDNS]`**. Confirma-se, agora sem ressalvas, que os três canais de fallback ficaram completamente silenciados.

**Resultado:** Fase 6 fechada do lado defensivo, com confirmação completa. LLMNR, NBT-NS e mDNS desligados e verificados na configuração do Windows 11; os três provados silenciados por captura direta do Responder ativamente à escuta — zero linhas de envenenamento em qualquer um dos três canais.

**Nota de coerência — RESOLVIDA (2026-09-18):** a GPO chamava-se inicialmente `Hardening-Desligar-LLMNR-NBTNS`, mas na prática só o LLMNR ficou implementado na GPO; o NBT-NS e o mDNS foram aplicados diretamente no cliente, por falta de definição nativa no ADMX deste Server. Renomeada para `Hardening-Desligar-LLMNR`, no GPMC do Windows Server, para o nome refletir exatamente o que a política faz. Acrescentado também um comentário na GPO (Group Policy Management Editor → propriedades do nó raiz → aba "Comment") a explicar que o NBT-NS e o mDNS são geridos do lado do cliente, não por esta GPO.

**Gravidade e impacto real:** a captura de um hash NTLMv2 por LLMNR/NBT-NS/mDNS poisoning não exige nenhuma credencial prévia — basta o atacante estar no mesmo segmento de rede e a vítima procurar um nome que não resolve. É dos vetores mais fiáveis numa rede Windows interna, precisamente porque explora um comportamento de origem do sistema, não uma falha corrigível por patch.

**Consequência para a vítima / organização real (proposta — confirma/ajusta):** numa organização, este é tipicamente o *primeiro elo* de um ataque interno: um hash capturado numa manhã pode ser quebrado offline (se a password for fraca) ou reencaminhado em tempo real (NTLM relay) para autenticar noutra máquina sem sequer o quebrar — a base de muitos ataques de ransomware. O que o torna traiçoeiro é ser silencioso e não depender de nenhum erro de configuração "visível": o LLMNR/NBT-NS/mDNS vêm ligados de origem, e uma empresa que nunca os desligou por política está exposta mesmo estando "bem gerida". A defesa, por isso, é uma decisão ativa de hardening (desligar os três fallbacks + SMB signing + passwords fortes), não um patch que chega sozinho.

**Autoavaliação — Consigo explicar isto a alguém?** Sim, depois de trabalhar a peça central em diálogo (não era óbvia à primeira). Por palavras próprias: quando o computador não sabe uma morada, em vez de gritar a pergunta para toda a rede e aceitar a primeira resposta sem verificar quem respondeu, desligar LLMNR/NBT-NS/mDNS faz com que ele simplesmente desista em silêncio — sem grito, o Responder não tem a quem responder primeiro. Não há nenhuma falha técnica a corrigir; é assim que o protocolo foi desenhado desde sempre, numa altura em que isto não era visto como risco.

**English summary:** Session 6.9 closes Phase 6 on the defensive side. Part A: a defensive balance mapping each Phase 6 attack (6.1 recon, 6.2 BloodHound, 6.3 Kerberoasting, 6.4 AS-REP Roasting, 6.6 Responder) to the concrete defense that prevents or detects it earliest, plus a conceptual note on the excluded 6.7/6.8. Part B: implemented and tested the Responder defense — disabling name-resolution fallbacks. LLMNR was disabled via a GPO ("Turn off multicast name resolution" = Enabled) linked to the Lab/Computadores OU; NBT-NS was disabled on the Windows 11 adapter (WINS tab), because this Server's ADMX lacks the native "Configure NetBIOS settings" policy. Verified EnableMulticast=0 and TcpipNetbiosOptions=2. A first Responder run proved LLMNR and NBT-NS silenced (only [MDNS] poisoning appeared, no [LLMNR]/[NBT-NS]), revealing mDNS as a residual third vector; mDNS was then disabled via EnableMDNS=0 in the same DNSClient policy key. A second run (\\testelab5) produced a clean "cannot access / check spelling" name-resolution failure instead of the earlier credential prompt. A follow-up session (2026-09-18) cleared two stale Responder instances and repeated the test with Responder actively listening (\\testelab6): zero poisoning lines appeared across all three channels ([LLMNR]/[NBT-NS]/[MDNS]), and Windows again showed the clean resolution-failure dialog — the definitive confirmation that all three fallbacks are silenced.
...
6. **Segundo problema encontrado — motor "morto" sem a GUI refletir isso:** Depois da correção da interface (ponto 4), o motor continuou a comportar-se de forma instável: parava sozinho de vez em quando e reiniciava, até que, por volta das 12:20, parou de vez e nunca mais voltou a arrancar sozinho. O problema só era visível na shell — a GUI (ícone de estado em Administration) continuava a mostrar "a correr", mesmo sem processo nenhum vivo. Só foi descoberto ao entrar por SSH na shell do OPNsense e correr `ps aux | grep suricata`: nenhum processo, mais de 2 horas depois da última paragem.
   - **Causa provável:** o log operacional (`suricata_20260920.log`) mostrou o motor a receber repetidos sinais de paragem (`Signal Received. Stopping engine.`) ao longo da manhã, reiniciando sozinho algumas vezes — até parar de vez sem mais tentativas automáticas. Este padrão (parar/reiniciar repetidamente num curto espaço de tempo, seguido de desistência total) é típico de uma proteção do sistema operativo contra "reinícios em loop" (flapping): quando um serviço para e arranca demasiadas vezes seguidas num curto período, o sistema por vezes deixa de o tentar reiniciar automaticamente.
   - **Resolução:** forçado um ciclo manual de paragem + arranque (⬛ seguido de ▶️) na GUI, e desta vez confirmado como processo real e vivo através da shell (`ps aux`, mostrando `--pcap=em1`, a interface correta — não a `--pcap=em0` do bug anterior). Um scan de teste a partir do Kali (`nmap -sC -p80 192.168.10.254`, scripts NSE por defeito) gerou de imediato **27 alertas** da assinatura `SID 2024364` ("ET SCAN Possible Nmap User-Agent Observed"), todos datados de hoje, corretamente atribuídos à interface `em1` (LAN) — confirmando o motor operacional de ponta a ponta.
...
**Resultado:** Wazuh e Sysmon confirmados 100% operacionais nas 4 máquinas. Suricata: estava completamente parado, foi reativado, um bug real de configuração (interface errada) foi encontrado e corrigido, e — ainda na mesma sessão — confirmou-se que o motor tinha voltado a parar sozinho por uma segunda razão distinta (reinícios em loop / "flapping"), corrigida com um ciclo manual de paragem/arranque. A prova final chegou: 27 alertas de tráfego real do lab, gerados por um scan de teste a partir do Kali. 7.0 fica registada como **fechada**.
...
**English summary:** Session 7.0 opens Phase 7 (Blue Team) by re-verifying the lab's detection stack before evaluating it in 7.1. Wazuh: 2 of 4 agents were found disconnected (`ubuntu-wg`, `windows-server`); both recovered — `windows-server`'s own agent log showed a successful reconnect at 10:47:28 that the dashboard just hadn't refreshed yet. A separate, unrelated finding was logged: `windows-server` is throwing repeated AD trust-relationship warnings during FIM/SCA scans (likely VMware snapshot drift), noted as a pending item. Suricata (OPNsense) was found fully stopped; restarting it revealed a real configuration bug — the engine was bound to device `em0` (the `OPT1`/192.168.50.0 interface) instead of `em1` (`LAN`/192.168.10.0, the actual lab network), despite the settings UI showing `LAN` selected. Fixed via Clear All + reselect LAN + Apply; the engine no longer self-stops after ~10-20s. Later in the same session, direct SSH inspection of Suricata's eve.json (bypassing the GUI's unreliable Alerts tab) revealed the engine had actually died completely — no process at all (confirmed via `ps aux`), despite the GUI icon still showing it as running. The operational log showed a repeated "Signal Received. Stopping engine" pattern through the morning, auto-restarting a few times before giving up for good — consistent with a restart-loop ("flapping") protection kicking in. A manual stop/start cycle in the GUI, verified as a real live process via `ps aux`, fixed it. A follow-up scan (`nmap -sC -p80`) from Kali produced 27 fresh alerts (SID 2024364) correctly tagged to the LAN interface (`em1`), providing the final end-to-end proof. Also noted: OPNsense's clock is UTC while the rest of the lab is WEST (UTC+1) — relevant for log correlation in 7.1.
...
**Resultado:** regra `100012` validada de ponta a ponta. A linha 18 do mapa de cobertura MITRE ATT&CK (Sessão 7.1) passa de 🟡 (alerta genérico) a 🟢 (deteção dedicada) — atualização já feita no ficheiro `mapa-cobertura-mitre-attack.md`. Sessão 7.2 fica parcialmente aberta — o plano previa escolher 1-2 lacunas; esta foi a primeira, ficando as restantes (linhas 🔴/🟡 da 7.1) disponíveis para uma sessão futura, se o Pedro quiser continuar a fechar mais alguma antes de avançar para a 7.3.

**Nota GRC:** A regra `100012` fecha uma lacuna de monitorização documentada (ISO/IEC 27001:2022 A.8.16 — Monitoring activities) e responde diretamente à exigência de deteção de incidentes da NIS2 (Art. 21). Antes desta regra, um pedido AS-REP Roasting real caía num nível de severidade genérico (5) indistinguível de tráfego Kerberos de rotina — um SOC com volume real de alertas provavelmente nunca o priorizaria para investigação. A subida para nível 10, com descrição e MITRE `T1558.004` dedicados, muda isso: passa a competir por atenção com outros alertas de alta severidade, como devia.

**Consequência para a organização real (perspetiva vítima):** Uma conta com pré-autenticação Kerberos desativada (como a `svc_legacy` usada aqui) é tipicamente uma conta legada ou mal configurada — exatamente o tipo de alvo "barato" que um atacante procura para extrair um hash e tentar quebrá-lo offline, sem precisar sequer de uma tentativa de login falhada visível. Antes desta regra, esse ataque passaria despercebido durante todo o tempo que o atacante precisasse para a fase offline de cracking — só seria descoberto depois de a password já estar comprometida (ex. através do uso indevido da conta). Com a `100012` a gerar um alerta de nível 10 assim que o pedido acontece, a janela de deteção passa a coincidir com o próprio momento do ataque, não com as suas consequências — dando à equipa de segurança a oportunidade de rodar a password ou desativar a conta antes de o hash ser quebrado.
...
## Entrada #101 — Sessão 7.2 (continuação): linha 15 já estava coberta — correção ao mapa, não regra nova (Enumeração de AD sem credenciais / ANONYMOUS LOGON)

**Data:** 2026-09-23

**Máquinas ligadas:** Kali Linux (192.168.10.10, a gerar a sessão SMB nula), Windows Server/DC (192.168.10.1, agente `windows-server`), Wazuh Manager (192.168.10.30).

**Objetivo:** Continuar a Sessão 7.2 (Fase 7, Blue Team) — segunda lacuna escolhida do mapa de cobertura da 7.1: a linha 15 ("Enumeração de AD sem credenciais", marcada 🟡, evidenciada pela sessão SMB nula da Entrada #89). A intenção inicial era escrever uma regra Wazuh dedicada, tal como na Entrada #100, para distinguir o evento 4624 com `targetUserName: "ANONYMOUS LOGON"` do tráfego de logon normal.

### O achado: não era preciso escrever nada — a lacuna já não existia

Antes de escrever a regra, seguiu-se a mesma disciplina da Entrada #100: confirmar o campo exato num evento real. Primeiro procurado na `archives.json` (nenhum resultado — a Entrada #89 já tinha rodado da retenção), depois gerado um evento fresco a partir do Kali com o mesmo comando exato da Entrada #89 (`netexec smb 192.168.10.1`), confirmando de novo `Null Auth: True`.

Ao procurar o evento fresco no `archives.json`, um primeiro `grep` apanhou por engano o evento de logoff (4634), não o de logon (4624) — ambos partilham o mesmo `targetUserName: "ANONYMOUS LOGON"`. Corrigido o filtro para incluir também `"eventID":"4624"`, apareceu um resultado inesperado: o evento já vinha classificado pela regra `92652` (nível 6), não pela genérica `60106` que a análise da Sessão 7.1 tinha previsto com base numa leitura estática do ficheiro de regras.

**A regra `92652` é uma regra de fábrica do próprio Wazuh** (o mesmo padrão "filha da `92651`" já conhecido da investigação das Entradas #95/#100 — Kerberoasting e AS-REP Roasting), com descrição **"Successful Remote Logon Detected - User:\ANONYMOUS LOGON - NTLM authentication, possible pass-the-hash attack"**, mapeada a `T1550.002` (Pass the Hash) e `T1078.002` (Domain Accounts), com as táticas Defense Evasion, Lateral Movement, Persistence, Privilege Escalation e Initial Access.

**Causa raiz do erro do mapa da 7.1:** a análise original verificou a regra `60194` (nível 0, "ignorada", filtra por `subjectUserName`) e concluiu, corretamente, que essa regra específica não se aplica a este cenário — mas nunca chegou a procurar por outras regras de fábrica que pudessem intercetar o evento antes da `60106` genérica (a mesma lição já registada na skill `validar-regra-wazuh`, ponto 6, mas aqui ao contrário: desta vez a regra "escondida" ajuda, não atrapalha). Sem um evento real gerado e testado, esta lacuna na análise nunca teria aparecido — prova de que uma auditoria de cobertura baseada só em leitura estática do ruleset (`grep` a ficheiros XML) tem um ponto cego real, mesmo quando feita com cuidado.

**Validação confirmada nas três camadas:**
1. `archives.json` — evento 4624, `targetUserName: "ANONYMOUS LOGON"`, classificado pela regra `92652`.
2. `alerts.json` — `sudo grep '"id":"92652"' /var/ossec/logs/alerts/alerts.json` confirma o alerta gerado.
3. **Dashboard** (Threat Hunting → Events, `agent.name: windows-server and rule.id: 92652`, "Last 24 hours") — confirmado visualmente: 1 hit, nível 6, descrição completa, agente `windows-server`.

**Nota GRC:** Do ponto de vista de governação, este é um caso raro e positivo: uma auditoria de cobertura (ISO/IEC 27001:2022 A.8.16 — Monitoring activities) identificou uma lacuna aparente que, ao ser testada na prática, se revelou já coberta — reforça o valor de complementar a revisão documental/estática de controlos com testes funcionais reais (NIS2 Art. 21, gestão de risco de cibersegurança) antes de reportar uma lacuna como aberta. Evita também um esforço de engenharia desnecessário: escrever e manter uma regra redundante com a `92652`.

**Consequência para a organização real (perspetiva vítima):** Uma sessão SMB nula, isoladamente, é ambígua — pode ser uma ferramenta de scanning legítima ou o primeiro passo de reconhecimento de um atacante (já discutido na Entrada #89). O que esta sessão confirma é que, pelo menos quando essa sessão nula é seguida de uma autenticação NTLM anónima bem-sucedida, o SOC recebe hoje um alerta específico, já correlacionado com técnicas de movimento lateral e Pass-the-Hash — não fica enterrado como "Windows Logon Success" genérico. Numa organização real, isto significa que um analista de SOC veria este evento destacado, com contexto de MITRE ATT&CK pronto, permitindo uma triagem mais rápida do que aconteceria se a lacuna assumida pelo mapa fosse real.

**Resultado:** Segunda linha da Sessão 7.2 "tratada" — mas, ao contrário da 100012, o valor aqui não foi escrever uma regra nova, foi corrigir um erro genuíno no mapa de cobertura da 7.1 (linha 15, corrigida de 🟡 para 🟢, com nota da correção e da lição metodológica). Sessão 7.2 fica com duas linhas tratadas (18 e 15); restam ainda várias 🔴/🟡 no mapa para uma sessão futura, se o Pedro quiser continuar antes de avançar para a 7.3.

**English summary:** Continuing Session 7.2, the second gap chosen was row 15 of the 7.1 coverage map (AD enumeration without credentials, the anonymous SMB session from Entry #89), marked 🟡. The intent was to write a dedicated Wazuh rule for event 4624 with `targetUserName: "ANONYMOUS LOGON"`. Following the same discipline as Entry #100, a fresh event was generated (re-running `netexec smb 192.168.10.1` from Kali, exactly as in Entry #89) since the original had rotated out of retention. Searching `archives.json`, an unexpected result turned up: the event was already classified by rule `92652` (level 6) — a genuine Wazuh factory rule ("Successful Remote Logon Detected - User:\ANONYMOUS LOGON - NTLM authentication, possible pass-the-hash attack", mapped to MITRE T1550.002/T1078.002) — not the generic `60106` the 7.1 map's static rule-file review had assumed. Root cause of the map's error: the original analysis correctly ruled out rule `60194` (level 0, checks the wrong field) but never searched for other factory rules that might intercept the event first — a blind spot only a live-fired test event could reveal. Validated end-to-end: decoded and correctly classified in `archives.json`, alerted in `alerts.json`, and visually confirmed in the Dashboard (1 hit, level 6, agent `windows-server`). No new rule was written; instead, the 7.1 coverage map's row 15 was corrected from 🟡 to 🟢, with a note on the methodological lesson: a static ruleset review, however careful, cannot fully substitute for a live test.
...
## Entrada #104 — Sessão 7.4: Resposta a incidentes — playbook + simulação completa (FTP anónimo → RCE via web shell)

**Data:** 2026-09-24

**Máquinas ligadas:** OPNsense, Servidor Vulnerável (`192.168.10.101`), Kali Linux (Atacante, `192.168.10.10`), VM Wazuh (`192.168.10.30`).

**Objetivo:** Praticar o ciclo completo de resposta a incidentes (detetar → triar → conter → erradicar → recuperar → lições aprendidas) sobre um ataque real já feito no lab (FTP anónimo + Apache mal configurado = RCE via web shell, Entradas #59-60), com o Wazuh a vigiar em tempo real, e produzir um playbook reutilizável (`playbook-resposta-incidentes.md`).

**Alternativas consideradas:**
- Escolhido o cenário FTP anónimo → RCE em vez do Kerberoasting: já tinha deteção validada no nível mais alto do lab (regra `100013`, nível 12, Entrada #102), o web shell ainda estava ativo no servidor (confirmado na Entrada #102, "mais de um mês depois"), e o âmbito ficava contido a uma só VM, deixando mais tempo para o ciclo de resposta em si.

**Ação executada:**
1. **Detetar:** repetido o pedido `curl "http://192.168.10.101/shell.php?cmd=whoami"` — erro inicial de porta (a porta 80 é o container Docker do DVWA, sem ligação ao FTP; o web shell real está no Apache autónomo da porta 8080, montado na Entrada #60). Corrigido para `curl "http://192.168.10.101:8080/shell.php?cmd=whoami"` — RCE confirmado (`www-data`). Alerta `100013` (nível 12, MITRE T1505.003, tática Persistence) confirmado no Dashboard, 1 hit, `16:45:48`.
2. **Triar:** evento expandido no Dashboard (`data.srcip: 192.168.10.10`, `data.url: /shell.php?cmd=whoami`, `full_log` completo). Cruzado com o `tail` do `/var/log/apache2/access.log` no próprio servidor — mesma e única linha, verdadeiro positivo confirmado. Investigada e explicada uma aparente inconsistência (156 bytes no log vs. 9 bytes no `Content-Length` do `curl`) — o `LogFormat combined` do Ubuntu regista bytes totais (`%O`, cabeçalhos + corpo), não só o corpo (`%b`); não havia segundo pedido escondido.
3. **Conter (tentativa 1, falhada):** regra de bloqueio criada no OPNsense (Firewall → LAN, `Block` de `192.168.10.10` para `192.168.10.101`), aplicada. Testado com novo `curl` — continuou a devolver `200 OK`, sem qualquer bloqueio. Confirmado no Live View do OPNsense: nenhuma entrada de log para este tráfego, prova de que os pacotes nunca chegaram ao router. Causa: Kali e Servidor Vulnerável estão no mesmo segmento L2 da VMware ("Ciber", um LAN Segment) e na mesma sub-rede `/24` — tráfego intra-segmento nunca atravessa o gateway, por isso uma regra na interface LAN do OPNsense não tem efeito nenhum sobre ele.
4. **Conter (tentativa 2, bem-sucedida):** pivô para conteção ao nível do próprio host — `sudo iptables -A INPUT -s 192.168.10.10 -p tcp --dport 8080 -j DROP` no Servidor Vulnerável (com um desvio à parte: três tentativas de password sudo erradas antes de acertar). Testado com `curl` do Kali — `Ligação expirada` (timeout), confirmando o bloqueio eficaz.
5. **Erradicar:** listagem de `/srv/ftp/upload/` revelou **quatro** web shells, não um: `shell.php` (original, Entrada #60, 22/08), `shell3.php` e `shell4.php` (26/08), `shell5.php` (19/09). Confirmados por `cat` como web shells reais (`system()`/`shell_exec()` sobre `$_GET["cmd"]`). Hashes SHA-256 confirmaram que `shell.php`, `shell4.php` e `shell5.php` são bit-a-bit o mesmo ficheiro reenviado com nomes diferentes em sessões diferentes; `shell3.php` é uma variante distinta. Todos preservados (cópia local) antes de apagados com `sudo rm`. Causa raiz corrigida em duas camadas: (a) `anon_upload_enable=NO` e `anon_mkdir_write_enable=NO` em `/etc/vsftpd.conf`, serviço reiniciado (leitura anónima mantida, só a escrita foi desativada); (b) `php_admin_flag engine off` inserido no bloco `<Directory /srv/ftp/upload>` de `/etc/apache2/sites-available/000-default.conf`, validado com `apache2ctl configtest` e testado localmente (`curl http://localhost:8080/<ficheiro-teste>.php` devolveu o código-fonte em vez de o executar).
6. **Recuperar:** regra `iptables` de conteção removida (já não necessária, causa raiz corrigida). Teste final decisivo: repetido o ataque completo do zero a partir do Kali — login FTP anónimo aceite (`230 Login successful`, leitura mantida), mas `put shell.php` devolveu `550 Permission denied`. Confirma que a vulnerabilidade está corrigida na origem, não só temporariamente mascarada pelo bloqueio de IP já removido.
7. **Playbook escrito:** `playbook-resposta-incidentes.md`, com uma checklist genérica reutilizável por fase, seguida de como cada uma foi aplicada em concreto neste incidente.

**Resultado:** Ciclo completo de resposta a incidentes praticado de ponta a ponta sobre um incidente real e verificável, incluindo uma falha genuína de conteção (não simulada) diagnosticada e corrigida em tempo real, e um playbook reutilizável entregue.

**Deduções e raciocínio:** O momento mais valioso da sessão não foi nenhum dos passos que correram bem à primeira — foi a falha da conteção de rede. É fácil, num exercício de lab, assumir que aplicar uma regra de firewall resolve o problema só porque a interface confirma "Apply Changes" sem erro; só o teste funcional (repetir o `curl`) e depois a confirmação na fonte (Live View do OPNsense, sem entradas) revelaram que a regra nunca tinha sequer sido avaliada. Isto espelha diretamente a lição já registada nas Fases 3 e 6 sobre a "Ciber" ser um LAN Segment do VMware (comportamento de hub/L2 plano) — mas agora aplicada a um cenário de defesa, não só de captura de tráfego: um firewall de perímetro não protege comunicação dentro do mesmo segmento, um princípio de rede que se mantém válido fora deste lab. A descoberta de quatro web shells em vez de um, só por se ter olhado explicitamente para a pasta em vez de assumir, reforça a mesma disciplina já várias vezes praticada neste projeto (Sessão 7.3): nunca aceitar a primeira confirmação como resposta completa.

**Consigo explicar isto a alguém?** Sim — o ciclo completo (detetar → triar → conter → erradicar → recuperar → lições aprendidas), incluindo porque é que a primeira tentativa de conteção falhou e o que isso ensina sobre topologia de rede.

**Como nos podemos defender / lição operacional:** Para conteção de tráfego intra-segmento, um firewall de perímetro (gateway) não chega — é preciso segmentação real ao nível da rede virtual (VLANs, ou um switch com aprendizagem MAC em vez de um LAN Segment/hub) para que o tráfego entre hosts do mesmo segmento possa ser filtrado a meio, ou conteção ao nível do próprio host (firewall local, como fizemos aqui) como alternativa imediata quando a topologia não permite o primeiro. Novamente, cadeias de ataque com múltiplas falhas empilhadas exigem múltiplas correções — corrigir só uma camada deixa a porta meio aberta.

**Gravidade e impacto real (num cenário empresarial):** Uma organização que só tivesse conteção ao nível do firewall de perímetro, sem perceber que segmentos internos "planos" não passam por ele, teria uma falsa sensação de segurança exatamente como aconteceu aqui na primeira tentativa — a equipa via a regra "aplicada" no ecrã e podia razoavelmente assumir que o incidente estava contido, enquanto o atacante continuava a ter acesso total. É um erro silencioso: não há mensagem de erro nenhuma a dizer "esta regra não serve para nada aqui", só o teste funcional revela a falha. É também um argumento a favor de segmentação de rede real (VLANs, firewalls internos) em ambientes empresariais, não só um perímetro externo — a maioria das violações reais já começa dentro da rede (endpoint comprometido, insider), onde um firewall só de perímetro nunca chega a intervir.

**Consequência para a vítima / organização real (perspetiva vítima):** O tempo entre a conteção "aplicada mas ineficaz" e a descoberta de que não funcionou é tempo extra que o atacante tem para continuar a agir — neste exercício foram só minutos, mas numa organização real, se a equipa confiar na regra e passar à fase seguinte (erradicação) sem testar a conteção primeiro, o atacante pode continuar ativo durante toda essa janela sem que ninguém perceba. Isto reforça um princípio central de resposta a incidentes: cada fase deve ser validada antes de se avançar para a seguinte, nunca assumida como bem-sucedida só porque a ação foi executada sem erro aparente.

**Domínios relacionados:** NIS2 (gestão e notificação de incidentes, Art. 21); ISO/IEC 27001:2022 A.5.24-A.5.28 (gestão de incidentes de segurança da informação); Security+ D4 (Operações — resposta a incidentes); MITRE ATT&CK T1505.003 (Web Shell).

**Próximos passos:** Sessão 7.5 — Baseline de hardening consolidado (juntar as defesas espalhadas pelo projeto num só documento).

**English summary:** Full incident-response cycle practiced end-to-end on a real, verifiable incident (the FTP-anonymous-to-RCE chain from Entries #59-60), with Wazuh watching live. Detection confirmed via rule `100013` (level 12, MITRE T1505.003). Triage cross-checked the SIEM alert against the raw Apache log and resolved an apparent size discrepancy (Apache's combined LogFormat logs total bytes including headers, not just body). The first containment attempt — an OPNsense firewall rule — genuinely failed: Kali and the target VM share the same flat L2 VMware LAN Segment and subnet, so intra-segment traffic never reaches the gateway, confirmed by an empty OPNsense Live View log. Pivoted to host-level containment (`iptables DROP` on the target), which worked. Eradication found four web shells, not the one expected — three were byte-identical re-uploads of the same file across different dates, confirmed via SHA-256 — and fixed two stacked root causes (anonymous FTP write, and PHP execution in the upload directory) rather than just removing the files. Recovery was proven, not assumed: after removing containment and fixing the root cause, a full repeat of the original attack chain from scratch failed exactly where it should (login succeeded, upload was denied). A reusable playbook (`playbook-resposta-incidentes.md`) was written from this concrete walkthrough. The standout lesson: a firewall rule that "applies" without error can still do nothing at all, and only a functional retest — not the absence of an error message — proves containment actually worked.

## Entrada #105 — Sessão 7.6 (Parte 1): Balanço da Fase 7 — o que mudou na maturidade defensiva do lab

**Data:** 2026-09-25

**Objetivo:** Fechar a Fase 7 (Blue Team: Deteção e Resposta) com o mesmo espírito da Entrada #98 (fecho da Fase 6) — não avançar para a fase seguinte sem primeiro olhar para trás e nomear, com honestidade, o que mudou de facto na maturidade defensiva do laboratório, não só listar o que foi feito.

**Balanço, sessão a sessão:**

- **7.0 — Baseline de visibilidade (Entrada #99):** antes desta sessão, a stack de deteção (Wazuh, Sysmon, Suricata) era assumida como viva, sem confirmação recente. A verificação revelou que não estava — dois agentes Wazuh desconectados e o Suricata completamente parado, com um bug real de configuração por trás (interface errada) e uma segunda falha distinta (motor "morto" com a GUI a mostrar "a correr"). Mudança de maturidade: de "a deteção existe" para "a deteção foi confirmada viva, com prova, não assumida".
- **7.1 — Mapa de cobertura MITRE ATT&CK (`mapa-cobertura-mitre-attack.md`):** primeira vez que o lab teve uma auditoria sistemática, técnica a técnica, dos 19 ataques já feitos — com células vermelhas honestas, não só um resumo dos sucessos. Mudança de maturidade: de "sabemos que temos deteção" para "sabemos exatamente o que a deteção apanha, e o que não apanha".
- **7.2 — Fechar lacunas prioritárias (Entradas #100-102):** duas regras Wazuh novas e reais (AS-REP Roasting, RCE via web shell — esta última a regra de maior severidade do lab), e uma correção ao próprio mapa (uma lacuna que afinal já estava coberta, descoberta só por teste ao vivo). Mudança de maturidade: de "mapa teórico" para "mapa corrigido por evidência real, incluindo os seus próprios erros".
- **7.3 — Threat hunting proativo (Entrada #103):** a descoberta mais subtil da fase — um alerta que dispara mas atribui o nome errado à técnica (BloodHound classificado como "possível pass-the-hash"). Mudança de maturidade: de "alerta = resolvido" para "alerta = ponto de partida a validar", a distinção entre um analista júnior e um mais experiente.
- **7.4 — Resposta a incidentes (Entrada #104):** primeira vez que o lab praticou o ciclo completo (detetar → triar → conter → erradicar → recuperar → lições aprendidas) sobre um incidente real, incluindo uma falha genuína de conteção (não simulada) diagnosticada e corrigida em tempo real. Mudança de maturidade: de "sabemos detetar" para "sabemos responder", com um playbook reutilizável como resultado concreto.
- **7.5 — Baseline de hardening consolidado (`hardening-baseline.md`):** nove defesas espalhadas por quatro meses de registo reunidas num só documento, sete delas reconfirmadas ao vivo (não só relidas) — e uma correção real encontrada pelo caminho (a exceção de porta 80 do egress filtering tinha revertido sozinha desde a Entrada #92). Duas lacunas documentadas como aceitação de risco consciente, não silêncio. Mudança de maturidade: de "defesas espalhadas pelo histórico" para "um retrato único e verificado do estado real de segurança do lab".

**Resultado:** A Fase 7 não acrescentou só mais ataques nem só mais regras — mudou a **pergunta por defeito** feita a cada defesa e a cada alerta, em todas as sessões: não "está configurado?", mas "está mesmo a funcionar, agora, e como se prova isso?". Essa pergunta gerou, por três vezes distintas nesta fase (7.0, 7.3, 7.5), a descoberta de que algo dado como certo (Suricata vivo, um alerta corretamente identificado, uma regra de firewall corrigida) afinal não estava assim.

**Deduções e raciocínio:** Olhando para trás desde a Fase 5 (onde as primeiras defesas foram implementadas), o percurso tem uma direção clara: Fase 5-6 = **implementar** defesas; Fase 7 = **verificar, corrigir e responder** quando essas defesas falham ou mentem sobre o seu próprio estado. É a diferença entre montar um sistema de segurança e operá-lo como um SOC operaria — a maturidade real de uma organização não se mede pelo número de controlos que tem no papel, mede-se pela frequência com que alguém testa se esses controlos continuam a funcionar.

**Nota GRC:** Este balanço, por si só, é um artefacto de governança — mapeia diretamente para o ciclo **Plan-Do-Check-Act** da ISO/IEC 27001 (a Fase 7 é sobretudo o "Check": verificar se os controlos implementados nas fases anteriores continuam eficazes) e para a exigência de melhoria contínua da gestão de risco da NIS2 (Art. 21). Um programa de segurança que nunca revisita as suas próprias defesas está, por definição, a operar sem essa fase — por mais controlos que tenha implementado.

**Consequência para a organização real (perspetiva vítima):** A lição mais cara desta fase, se aplicada a uma organização real, é esta: dos sete pontos reconfirmados no `hardening-baseline.md`, dois já não estavam corretos (a porta 80 do egress filtering, e a versão inicial dos agentes/Suricata na Entrada #99) — ambos sem que ninguém tivesse mudado nada deliberadamente. Uma empresa que confie em auditorias pontuais, feitas uma vez e nunca repetidas, está a assumir que o estado de segurança é estático — quando a evidência aqui, repetida três vezes na mesma fase, mostra precisamente o contrário: configurações revertem, processos morrem em silêncio, alertas mentem sobre o que detetam. O custo de descobrir isto tarde (num incidente real, não numa auditoria) é sempre maior do que o custo de reverificar com regularidade.

**Domínios relacionados:** ISO/IEC 27001:2022 (ciclo PDCA, A.5.35 — revisão independente da segurança da informação); NIS2 (Art. 21, gestão contínua de risco); Security+ D4 (Operações de Segurança — deteção, resposta, hunting); transversal a toda a fase.

**Próximos passos:** Sessão 7.6 (Parte 2) — confirmar tudo committed e feito push para o GitHub, como fecho formal do arco completo do projeto (ataque → deteção → resposta → hardening). Ponte para o Projeto 2 (GRC/Governança), se for essa a direção seguinte confirmada no `estado-projeto-2026-08-27.md`.

**English summary:** Closing retrospective for Phase 7 (Blue Team), mirroring how Phase 6 was closed (Entry #98). Session by session: 7.0 found the detection stack wasn't actually alive despite being assumed so (two disconnected Wazuh agents, a fully-stopped Suricata with two distinct real bugs); 7.1 produced the lab's first honest, technique-by-technique detection coverage audit; 7.2 closed two real gaps with new Wazuh rules and caught the coverage map's own error on a third; 7.3 surfaced a more subtle failure mode — an alert that fires but misidentifies the technique; 7.4 practiced a full incident-response cycle on a real incident, including a genuine (not staged) containment failure diagnosed and fixed live; 7.5 consolidated nine defenses into one document, live-re-verifying seven of them and catching a real regression along the way (a firewall exception that had silently reverted). The throughline: Phase 7 didn't just add more attacks or more rules — it changed the default question asked of every defense, from "is it configured?" to "is it actually working right now, and how do we prove it?". That question caught real drift three separate times in this phase alone (7.0, 7.3, 7.5), which is itself the phase's central lesson: security posture is not static, and infrequent one-time audits will miss exactly the kind of silent regression demonstrated here.
...
- `screenshots/2026-09-08/entrada95-dashboard-4769-23hits-regra60106.png` — Wazuh Dashboard (Events), pesquisa `data.win.system.eventID: 4769`: 23 resultados, todos os eventos de rotina corretamente classificados pela regra genérica `60106` — confirma que o resto do pipeline continuava saudável enquanto se investigava o evento do ataque, que não aparece aqui por não ter (ainda) nenhuma regra associada (Entrada #95)
- `screenshots/2026-09-08/entrada95-dashboard-100011-confirmado.png` — Wazuh Dashboard (Events), pesquisa `rule.id: 100011`: 1 resultado, nível 10, "Kerberoasting: Pedido de TGS (Service Ticket) com encriptacao RC4...", confirmação visual final da resolução da Sessão 6.7 (Entrada #95)
